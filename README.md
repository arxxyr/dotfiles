...
```bash
chezmoi source-path ~/.zshrc      # 定位并编辑源文件，不直接改生成目标
chezmoi add ~/.config/xxx/conf    # 纳管新配置
chezmoi diff ~/.zshrc             # 预览本次目标
chezmoi apply ~/.zshrc            # 定向应用；随后手动提交，需要时推送
```

`autoCommit` / `autoPush` 均关闭，避免纳管操作夹带无关修改。
...
- `south-asia-translator` 保留原流程、脚本与术语表，仅将入口统一为 `SKILL.md`。
- `.chezmoidata.toml` 的 `agent_skills` 统一记录纳管与退役名单；`.chezmoiremove` 只删除退役的
  精确个人目录及链接，不使用会清空未知技能的 `exact_`。重新启用退役名称前必须先调整名单。
- `.agents/.skill-lock.json` 改为选择性迁移：仅移除纳管或退役技能的安装器登记，其他本机技能、
  偏好和未知字段保留。恢复由源文件与 `chezmoi apply` 完成，不靠安装器重新拉取旧技能。

首次迁移前，将 `~/.agents/skills`、`~/.claude/skills`、`~/.codex/skills`、`~/.openclaw/skills`、旧全局指令和锁文件
归档到技能发现目录以外；退役的整个个人目录会被删除，其他机器上的自定义内容应先回收到源仓库。
