...
2026-09-09，按后续授权迁出 `a-share-trend-entry`，删除 `team-swe`、`handoff`，其他技能不动。
用户另要求加入三客户端分工，并在提交后合并到 `master`、推送和清理其他分支。

- 研究内容迁入 `notes/research/a-share-trend-entry/`：四份参考逐字节不变，方法正文仅移除技能元数据，
  新增中文索引及历史资料说明；没有刷新行情或改写历史结论。迁移前内容可查 `d25069b`。
- `notes/` 由 Git 保存并由 chezmoi 忽略；归档没有技能入口，不部署到 home 或参与技能发现。
- 三个技能目录及九个客户端链接退役，纳管名单 13 → 10、退役名单 17 → 20；删除已无用途的
  `team-swe` 调用策略检查，保留系统验证器。其他十个技能源与三个 `cc-*` 启动器未修改。
- 共享指令增加分工：`cc-codex` 总管与方案设计，`cc-claude` 开发、自测及已授权部署，
  `cc-kimi` 很简单的低风险任务，最终评审和关键决策由 `cc-codex` 负责。此处是分工约定，不是启动器自动调度。
- 本机定向应用后，二十个退役名称在四个个人入口均不存在；保留的 229 个资源逐字节一致，
  三十个技能链接及两个全局链接有效，相关目标再次预览为空。
- 十项隔离测试、十个技能校验、Ruff 检查和格式检查、差异检查通过；新增测试在 Linux 主机上覆盖
  linux/darwin/windows 三个忽略模板分支，确认普通配置正常应用、归档不部署、已有目标笔记不被覆盖。
- 未参与实现的子 agent 独立复查迁移、调用分工、范围和测试，未发现遗漏或跨范围变更。
- 删除分支前已归档本机相关目录及完整 Git bundle；远程旧 `fix/cc-codex-app-proxy` 分支的
  `c28e96a` 经 `git cherry` 确认有等价补丁在 `master`，不将其误认为仍需重复合并的开发内容。

以上是提交前验证记录，不代表远程操作已经完成；最终提交、推送与分支状态以 Git 实际结果为准。
