...
- `cc-codex`：担任总管，负责需求澄清、方案设计、任务分配与结果整合。
- `cc-claude`：负责具体开发、自测，以及任务已授权的部署。
- `cc-kimi`：负责范围明确、风险低、无需关键设计取舍的很简单任务。
- 最终评审与关键决策由 `cc-codex` 负责；简单任务不强制串行调用全部工具。
- 交接时说明目标、约束和已有验证结果；分工不扩大修改、发布或部署授权。
